    ...
    <el-table :data="getData" style="width: 98%">
      ...
      <el-table-column type="expand" prop="goodsPic" label="图片展示">
         <template scope="scope">
                   <img :src="scope.row.goodsPic[0].path" />
        </template>
      </el-table-column>
      <el-table-column label="商品名称" prop="goodsTitle" min-width="120px"> </el-table-column>
      <el-table-column label="所属分类" prop="classTitle" min-width="80px">
        <template scope="scope">
          <span>{{ scope.row.goodsClass.classTitle }}</span>
        </template>
      </el-table-column>
      <el-table-column label="商品单价" prop="price" min-width="100px;">
        <template scope="scope">
      ...
      <el-table-column label="库存" prop="repositoryNum" width="80px">
      </el-table-column>
      </el-table-column>
...
import { prolist, gclist, editgoods, findgoods } from '@/service/getData'
export default {
  data() {
    return {
      checked: true,
